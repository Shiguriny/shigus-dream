...
object ActionRegistry {
    val ACTIONS = listOf(
        ...
        ActionSpec(
            id = "shigusdream:cinematic",
            name = "Cinematic",
            description = "Кинематографический режим: letterbox + скрытие HUD.",
            permission = "client.action.cinematic",
            schema = listOf(
                SchemaField(key = "duration", type = "int", required = true, min = 40.0, max = 24000.0, description = "Тики (20/с)"),
            ),
        ),
        ActionSpec(
            id = "shigusdream:camera_shake",
            name = "Camera Shake",
            description = "Тряска камеры цели: style fine|wide.",
            permission = "client.action.camera_shake",
            schema = listOf(
                SchemaField(key = "duration", type = "int", required = true, min = 10.0, max = 600.0, description = "Тики (20/с)"),
                SchemaField(key = "intensity", type = "float", min = 0.1, max = 3.0, description = "Сила"),
                SchemaField(key = "style", type = "string", allowedValues = listOf("fine", "wide"), description = "Тип тряски"),
            ),
        ),
        ActionSpec(
            id = "shigusdream:play_ambient",
            name = "Play Ambient",
            description = "Позиционный/зацикленный эмбиент-звук.",
            permission = "client.action.play_ambient",
            schema = listOf(
                SchemaField(key = "sound", type = "identifier", required = true, description = "Идентификатор звука"),
                SchemaField(key = "mode", type = "string", allowedValues = listOf("point", "follow"), description = "Точка или позиция игрока"),
                SchemaField(key = "x", type = "float", description = "X точки"),
                SchemaField(key = "y", type = "float", description = "Y точки"),
                SchemaField(key = "z", type = "float", description = "Z точки"),
                SchemaField(key = "interval", type = "int", min = 5.0, max = 1200.0, description = "Повтор каждые N тиков"),
                SchemaField(key = "duration", type = "int", required = true, min = 20.0, max = 24000.0, description = "Общая длительность"),
                SchemaField(key = "volume", type = "float", min = 0.0, max = 4.0, description = "Громкость"),
            ),
        ),
        ActionSpec(
            id = "shigusdream:scare",
            name = "Scare",
            description = "Случайная микро-пугалка: flash, blink, shake или шёпот за спиной.",
            permission = "client.action.scare",
            schema = listOf(
                SchemaField(key = "intensity", type = "float", min = 0.1, max = 2.0, description = "Сила испуга"),
            ),
        ),
        ActionSpec(
            id = "shigusdream:highlight",
            name = "Highlight",
            description = "Выделение игрока (glow) или блока (HUD-маркер) для цели.",
            permission = "client.action.highlight",
            schema = listOf(
                SchemaField(key = "what", type = "string", required = true, allowedValues = listOf("entity", "block"), description = "Кого/что выделять"),
                SchemaField(key = "entity", type = "string", maxLength = 16, description = "Ник игрока (для entity)"),
                SchemaField(key = "pos", type = "string", maxLength = 64, description = "Блок: x y z (для block)"),
                SchemaField(key = "duration", type = "int", required = true, min = 20.0, max = 12000.0, description = "Тики (20/с)"),
                SchemaField(key = "color", type = "string", maxLength = 7, description = "Цвет маркера блока, #RRGGBB"),
            ),
        ),
        ActionSpec(
